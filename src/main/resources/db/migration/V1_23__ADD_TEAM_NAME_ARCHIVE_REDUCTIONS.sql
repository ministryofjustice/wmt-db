...
CREATE OR REPLACE VIEW app.reductions_archive_view (om_name, hours_reduced, reduction_id, comments, last_updated_date, reduction_added_by, reduction_reason, start_date, end_date, reduction_status, team_name) AS
SELECT
    CONCAT(om.forename, ' ', om.surname) AS om_name, r.hours AS hours_reduced, r.id AS reduction_id, r.notes AS comments, r.updated_date AS last_updated_date, u.name AS reduction_added_by, rr.reason_short_name AS reduction_reason, r.effective_from AS start_date, r.effective_to AS end_date, r.status AS reduction_status, t.description AS team_name
