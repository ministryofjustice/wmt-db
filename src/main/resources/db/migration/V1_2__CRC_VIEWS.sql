...
CREATE OR REPLACE VIEW app.crc_capacity_view (total_points, available_points, reduction_hours, contracted_hours, effective_from, workload_report_id, count) AS
SELECT
    ...
    JOIN app.region AS r
        ON r.id = l.region_id
    WHERE LOWER(r.description) NOT LIKE LOWER('NPS%')
    GROUP BY wr.effective_from, wr.id;



CREATE OR REPLACE VIEW app.crc_caseload_view (link_id, name, region_name, grade_code, location, untiered, a3, a2, a1, a0, b3, b2, b1, b0, c3, c2, c1, c0, d3, d2, d1, d0, total_cases, count) AS
SELECT
    ...
    WHERE wr.effective_from IS NOT NULL AND wr.effective_to IS NULL AND LOWER(r.description) NOT LIKE LOWER('NPS%')
...
SELECT
    ...
    JOIN app.region AS r
        ON r.id = l.region_id
    WHERE LOWER(r.description) LIKE LOWER('NPS%')
    GROUP BY wr.effective_from, wr.id;
...
SELECT
    ...
    JOIN app.offender_manager_type AS omt
        ON omt.id = om.type_id
    WHERE wr.effective_from IS NOT NULL AND wr.effective_to IS NULL AND LOWER(r.description) LIKE LOWER('NPS%')
    GROUP BY r.id, r.description, omt.grade_code, tr.location;
