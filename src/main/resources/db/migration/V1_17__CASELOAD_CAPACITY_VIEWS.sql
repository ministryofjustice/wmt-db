...
SELECT
    ...
    JOIN app.region AS r
        ON r.id = l.region_id
    WHERE LOWER(r.description) LIKE LOWER('CPA%')
...
SELECT
    ...
    WHERE wr.effective_from IS NOT NULL AND wr.effective_to IS NULL AND LOWER(r.description) LIKE LOWER('CPA%')
...
SELECT
    ...
    JOIN app.region AS r
        ON r.id = l.region_id
    WHERE LOWER(r.description) NOT LIKE LOWER('CPA%')
    GROUP BY wr.effective_from, wr.id;
...
SELECT
    ...
    JOIN app.offender_manager_type AS omt
        ON omt.id = om.type_id
    WHERE wr.effective_from IS NOT NULL AND wr.effective_to IS NULL AND LOWER(r.description) NOT LIKE LOWER('CPA%')
    GROUP BY r.id, r.description, omt.grade_code, tr.location;
