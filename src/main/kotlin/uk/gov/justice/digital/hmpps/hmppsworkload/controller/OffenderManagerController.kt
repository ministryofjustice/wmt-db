...
class OffenderManagerController(
  ...
  fun getImpactOfAllocation(@PathVariable(required = true) teamCode: String, @PathVariable(required = true) staffId: Long, @RequestBody impactCase: ImpactCase): OffenderManagerPotentialWorkload {
    val potentialWorkload = offenderManagerService.getPotentialWorkload(teamCode, staffId, impactCase)
    if (potentialWorkload != null) {
      return OffenderManagerPotentialWorkload.from(potentialWorkload)
    }
